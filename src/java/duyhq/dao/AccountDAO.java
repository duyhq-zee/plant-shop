...
public class AccountDAO {
    ...
    public static Account getAccountById(int accId) {
        ...
        try {
            ...
            if (cn != null) {
                String sql = "SELECT accId, email, password, fullname, phone, status, role FROM Accounts WHERE status = 1 AND accId = ? COLLATE Latin1_General_CS_AS";
                PreparedStatement pst = cn.prepareStatement(sql);
                pst.setInt(1, accId);
                ResultSet rs = pst.executeQuery();
    ...
    public static Account getAccountByToken(String token) {
        ...
        try {
            ...
            if (cn != null) {
                String sql = "SELECT accId, email, password, fullname, phone, [status], role, [token] FROM Accounts WHERE status = 1 AND token = ? COLLATE Latin1_General_CS_AS";
                PreparedStatement pst = cn.prepareStatement(sql);
                pst.setString(1, token);
                ResultSet rs = pst.executeQuery();
